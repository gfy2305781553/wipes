...
Wipe.prototype.addEvent = function(){
	...
	this.cas.addEventListener(clickEvtName,function(evt){
		...
		that.moveX = that.device ? event.touches[0].clientX+_scrollLeft-that.getAllLeft(that.cas) : event.clientX+_scrollLeft-that.getAllLeft(that.cas);
		that.moveY = that.device ? event.touches[0].clientY+_scrollTop-that.getAllTop(that.cas) : event.clientY+_scrollTop-that.getAllTop(that.cas);
	...
	this.cas.addEventListener(moveEvtName,function(evt){
		...
		if (!that.isMouseDown) {
			...
			var x2 = that.device ? event.touches[0].clientX +_scrollLeft-that.getAllLeft(that.cas):event.clientX+_scrollLeft-that.getAllLeft(that.cas);
			var y2 = that.device ? event.touches[0].clientY +_scrollTop-that.getAllTop(that.cas):event.clientY+_scrollTop-that.getAllTop(that.cas);
